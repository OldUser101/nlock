...
in
{
  ...
  options.programs.nlock = {
    enable = mkEnableOption "" // {
      description = ''
        Whether to enable nlock.

        Note that PAM must be configured to enable nlock to perform
        authentication. The package installed through home-manager,
        will *not* be able to unlock the session without this
        configuration.

        On NixOS, this can be enabled using:
  ...
  config = mkIf cfg.enable {
    ...
    xdg.configFile."nlock/nlock.toml".source =
      let
